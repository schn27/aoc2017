...
function calc() {
	...
	input.split("\n").forEach(line => {
		...
		const r = tokens[0];
		const inc = (tokens[1] === 'inc' ? 1 : -1) * (+tokens[2]);
		const op = tokens[5];

		if (condition(op, readReg(regs, tokens[4]), +tokens[6])) {
			let v = readReg(regs, r) + inc;
			max = Math.max(v, max);
			regs[r] = v;
		}
	});

	return Object.values(regs).max() + " " + max;
}

function readReg(regs, r) {
	return (regs[r] != undefined) ? regs[r] : 0;
}
